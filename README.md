...
定期自動掃描 Binance 所有 USDT 交易對，篩選出符合「**多頭排列**」
...
GitHub Actions (每4小時排程一次)
     │  抓取 Binance API → 計算 SMA/KD/MACD → 篩選多頭排列 → 寫入 data/
     ▼
Binance 公開行情 API
```

跟股票版最大的差別：加密貨幣 24 小時交易、沒有休市日，Binance 的公開行情
API 不需要註冊帳號、不需要 API Key，速率限制也寬鬆很多，所以資料抓取比
台股版快很多、也穩定很多。
...
- **資料來源**：[Binance 公開行情 API](https://binance-docs.github.io/apidocs/spot/en/)（免key）
...
預設只會抓 **24小時成交額 >= 50萬 USDT** 的交易對（避免抓一堆沒人交易的
殭屍幣），全部跑完通常幾分鐘內就結束。可以用環境變數調整：

```bash
# 調整流動性門檻（USDT）
MIN_QUOTE_VOLUME_USDT=1000000 node scripts/updateData.js

# 只想先測試流程，只抓前20個幣，很快跑完
MAX_COINS=20 node scripts/updateData.js

# 改用其他K線週期（預設1d日線），例如4小時線
INTERVAL=4h node scripts/updateData.js
```
...
（不需要設定任何 Secret，Binance 公開 API 不用金鑰）
...
- 目前只抓 **現貨(spot) USDT交易對**，沒有涵蓋合約/期貨市場。
- 流動性過濾門檻（`MIN_QUOTE_VOLUME_USDT`）是簡單的24小時成交額判斷，不代表
  基本面篩選，小心殭屍幣、瞬間爆量幣誤判。
...
本專案僅供技術分析與程式學習用途，資料來源為 Binance 公開 API，可能有
